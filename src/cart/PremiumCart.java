...
public class PremiumCart extends CoffieCart {
	...
	private static final String PREMIUM_PAY_PERCENT = "premium_pay_precent";
	...
	public void makeMounthlyPayment() {
		Integer price = (Integer) Configuration.getInstance().getValueStore().get(PREMIUM_MOUNT_PRICE);
		if(balance.subtract(new BigDecimal(price)).intValue() > 0) {
			this.balance = balance.subtract(new BigDecimal(price));
			Integer claimCount = (Integer) Configuration.getInstance().getValueStore().get(PREMIUM_CLAIM_COUNT);
			claim = claimCount;
		}else {
